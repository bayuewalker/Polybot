...
class PolyClient:
    ...
    async def get_usdc_balance(self) -> float:
        """Fetch real USDC balance using multiple async methods."""
        if not self._acct:
            log.warning("No wallet connected — cannot fetch balance")
            return 0.0

        addr = self._acct.address
        log.info(f"Fetching USDC balance for {addr[:10]}…")

        # Method 1: Polygon JSON-RPC eth_call to USDC contracts
        ...
        for rpc_url in RPC_URLS:
            total = 0.0
            try:
                for contract_addr, label in USDC_CONTRACTS:
                    payload = {
                    ...
                    async with self.session.post(
                        rpc_url, json=payload, timeout=8
                    ) as r:
                        ...
                        if result and result != "0x":
                            ...
                            if bal > 0:
                                log.info(f"  {label}: ${bal} (via {rpc_url})")
                                total += bal

                if total > 0:
                    log.info(f"Total USDC: ${total}")
                    return total

            except Exception as e:
                log.debug(f"RPC {rpc_url} error: {e}")
                continue

        # Method 2: Polymarket CLOB balance endpoint
        try:
            ...
            async with self.session.get(url, params=params, timeout=8) as r:
                if r.status == 200:
                    ...
                    bal = float(data_resp.get("balance", 0))
                    if bal > 0:
                        log.info(f"Polymarket CLOB balance: ${bal}")
                        return round(bal / 1_000_000, 2)  # CLOB returns raw units
        except Exception as e:
            log.debug(f"CLOB balance error: {e}")

        # Method 3: PolygonScan API (free, no key needed for basic)
        try:
            USDC_NATIVE = "0x3c499c542cEF5E3811e1192ce70d8cC03d5c3359"
            url = f"https://api.polygonscan.com/api"
            params = {
                "module": "account", "action": "tokenbalance",
                "contractaddress": USDC_NATIVE,
            ...
            async with self.session.get(url, params=params, timeout=8) as r:
                if r.status == 200:
                    ...
                    if resp.get("status") == "1":
                        bal = round(int(resp["result"]) / 1_000_000, 2)
                        log.info(f"PolygonScan USDC balance: ${bal}")
                        return bal
        except Exception as e:
            log.debug(f"PolygonScan error: {e}")

        log.warning(f"All balance methods failed for {addr[:10]}")
        return 0.0
...
class WebAPI:
    ...
    async def handle_post_settings(self, req):
        ...
        # Mark as confirmed by user — bot will now trade
