"""
PolyBot v4.0 — Polymarket Trading Bot
======================================
NEW in v4.0:
  ✅ Built-in Web API — frontend Settings page controls the bot directly
  ✅ User sets private key, Telegram, risk from dashboard — no Railway needed
  ✅ Settings saved to settings.json — survive restarts
  ✅ /health and /settings endpoints
  ✅ CORS enabled — works with any frontend domain
  ✅ All v3.0 features kept
"""
...
class Config:
    ...
    def reload(self):
        s = load_file()
        g  = lambda key, env, default="": s.get(key) or os.getenv(env, default)
        gf = lambda key, env, default: float(s.get(key) or os.getenv(env, str(default)))
        gi = lambda key, env, default: int(s.get(key) or os.getenv(env, str(default)))
        gb = lambda key, env, default: str(s.get(key) or os.getenv(env, str(default))).lower() in ("true","1","yes")
...
class PolyClient:
    ...
    async def start(self):
        self.session = aiohttp.ClientSession(headers={"User-Agent": "PolyBot/4.0"})
...
class Risk:
    ...
    def stop(self, entry, side):
        return round(entry*(1-self.cfg.STOP_LOSS) if side=="YES" else entry*(1+self.cfg.STOP_LOSS),4)
    def check_stops(self):
        return [pid for pid,p in self.port.positions.items()
                if (p.side=="YES" and p.current_price<=p.stop_loss) or
                   (p.side=="NO"  and p.current_price>=p.stop_loss)]
...
class WebAPI:
    ...
    async def handle_get_settings(self, req):
        ...
        if "privateKey" in s and s["privateKey"]:
            safe["privateKey"] = "0x" + "•" * 20  # masked
    ...
    async def handle_post_settings(self, req):
        ...
        # Load existing to preserve private key if masked
        existing = load_file()
        ...
        # Reload config live — no restart needed!
        self.bot.cfg.reload()

        # Reinit wallet if private key changed
...
class TradingBot:
    ...
    async def start(self):
        ...
        log.info("PolyBot v4.0 started")
        await self.tg.send(
            "🤖 <b>PolyBot v4.0 Started</b>\n\n"
            "⚡ Arbitrage | 📰 News | 📊 MeanRev | 🔊 Volume | ⏰ Closing\n"
            "🌐 Web API: active — configure from dashboard\n\n"
    ...
    async def _update_positions(self, markets):
        ...
        for pid in self.risk.check_stops():
            await self.close(pid, "stop_loss")
