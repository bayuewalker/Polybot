...
class PolyClient:
    ...
    async def get_usdc_balance(self) -> float:
        """Fetch real USDC balance from Polygon wallet."""
        if not self._acct:
            return 0.0
        try:
            ...
            balance = round(raw / 1_000_000, 2)  # USDC has 6 decimals
            log.info(f"Real USDC balance: ${balance}")
            return balance
        except Exception as e:
    ...
    async def start(self):
        ...
        while True:
            try:
                if self.running:
                    await self.scan()
                    # Sync real balance every 5 scans
