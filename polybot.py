"""
PolyBot v5.2 — Polymarket Automated Trading Bot
================================================

LATEST UPDATE — v5.2 (2026-03-20):
  ✅ NEW: POLY_API_KEY, POLY_API_SECRET, POLY_API_PASSPHRASE support
  ✅ NEW: POLY_MAKER_ADDRESS — Polymarket deposit address for orders
  ✅ NEW: BASE_CAPITAL env var — reference starting capital
  ✅ NEW: _clob_headers() — authenticated headers for all CLOB requests
  ✅ NEW: place_order uses maker address + API auth headers
  ✅ NEW: Settings page shows Polymarket API Keys section
  ✅ NEW: Balance fetch now uses authenticated CLOB headers
  ✅ FIXED: Orders now use correct maker address (not just wallet)

v5.1 (2026-03-20):
  ✅ FIXED: Balance $0 now shown as real $0 (not faked)
  ✅ FIXED: Paper trading clearly labelled in ALL TG notifications
  ✅ NEW: is_paper_trading() — detects no-wallet mode
  ✅ NEW: _paper_tag() — adds ⚠️ Paper trading remark to every alert
  ✅ NEW: 5-layer balance fetch with auth headers
  ✅ FIXED: Duplicate signal — market-level cooldown key

v5.0 (2026-03-19):
  ✅ NEW: Bayesian signal model, Z-score filter, Fractional Kelly
  ✅ NEW: Sharpe, MDD, Profit Factor, VaR tracking
  ✅ NEW: PolyCop-style /positions and /status TG format
  ✅ NEW: Detailed trade alerts with P&L in $ and %

v4.2 (2026-03-19):
  ✅ FIXED: Bot crash, Telegram conflict, settings not saving
  ✅ FIXED: SL wrong direction for NO positions
  ✅ NEW: Bot starts STOPPED, TP/SL from frontend
...
class Config:
    ...
    def reload(self):
        ...
        self.API_KEY       = g("polyApiKey",  "POLY_API_KEY")        # from Polymarket profile
        self.API_SECRET    = g("polySecret",  "POLY_API_SECRET")     # from Polymarket profile
        self.API_PASSPHRASE= g("polyPass",    "POLY_API_PASSPHRASE") # from Polymarket profile
        self.MAKER_ADDRESS = g("makerAddress","POLY_MAKER_ADDRESS")  # Polymarket deposit address
        self.BASE_CAPITAL  = gf("baseCapital","BASE_CAPITAL", 0)     # starting capital reference
        ...
        self.BOT_SECRET    = g("apiSecret",   "API_SECRET","polybot123")
...
class PolyClient:
    ...
    async def start(self):
        self.session = aiohttp.ClientSession(headers={"User-Agent":"PolyBot/5.2"})
    ...
    def _clob_headers(self) -> dict:
        ...
        headers = {"User-Agent": "PolyBot/5.2"}
        if self._acct:
            headers["POLY_ADDRESS"] = self._acct.address
        if self.cfg.API_KEY:
            headers["POLY_API_KEY"]         = self.cfg.API_KEY
            headers["POLY_API_SECRET"]      = self.cfg.API_SECRET
            headers["POLY_API_PASSPHRASE"]  = self.cfg.API_PASSPHRASE
        if self.cfg.MAKER_ADDRESS:
            headers["POLY_ADDRESS"] = self.cfg.MAKER_ADDRESS
        return headers

    async def get_usdc_balance(self) -> float:
        if not self._acct: return 0.0
        addr = self.cfg.MAKER_ADDRESS or self._acct.address
        log.info(f"Fetching balance for {addr[:10]}…")

        # Method 1: Polymarket CLOB with full auth headers
        for params in [
            ...
            try:
                async with self.session.get(
                    ...
                    log.info(f"  CLOB {params} → {r.status}")
                    if r.status == 200:
                        d = await r.json()
                        log.info(f"  CLOB response: {d}")
                        for key in ["balance", "allowance", "amount"]:
                            raw = float(d.get(key, 0) or 0)
                            if raw > 0:
                                bal = raw / 1_000_000 if raw > 1000 else raw
                                log.info(f"  ✅ CLOB {key}: ${bal:.2f}")
                                return round(bal, 2)
            except Exception as e:
                log.warning(f"  CLOB error: {e}")

        # Method 2: Polymarket profile API (public, no auth needed)
        try:
            url = f"https://polymarket.com/api/profile/{addr}"
            async with self.session.get(url, timeout=8) as r:
                log.info(f"  Profile API status: {r.status}")
                if r.status == 200:
                    d = await r.json()
                    log.info(f"  Profile response: {d}")
                    for key in ["balance", "usdcBalance", "availableBalance"]:
                        val = d.get(key)
                        if val:
                            bal = round(float(val), 2)
                            if bal > 0:
                                log.info(f"  ✅ Profile {key}: ${bal:.2f}")
                                return bal
        except Exception as e:
            log.warning(f"  Profile API error: {e}")

        # Method 3: eth_call directly to USDC contract on Polygon (most reliable)
                ...
                try:
                    ...
                    async with self.session.post(rpc, json=payload, timeout=8) as r:
                        if r.status == 200:
                            ...
                            if result and result not in ("0x", "0x0") and len(result) > 10:
                                raw = int(result, 16)
                                bal = round(raw / 1_000_000, 2)
                                log.info(f"  eth_call {rpc[-20:]} → ${bal:.2f}")
                                if bal > 0:
                                    total += bal
                                    break  # found balance for this contract
                except Exception as e:
                    log.debug(f"  RPC {rpc[-20:]}: {e}")

        if total > 0:
            log.info(f"  ✅ eth_call total: ${total:.2f}")
            return round(total, 2)

        # Method 4: PolygonScan token balance API (free tier)
        try:
            for contract in CONTRACTS:
                ...
                params = {
                    ...
                    "apikey": os.getenv("POLYGONSCAN_KEY", "YourApiKeyToken")
                }
                async with self.session.get(url, params=params, timeout=8) as r:
                    if r.status == 200:
                        d = await r.json()
                        log.info(f"  PolygonScan {contract[:10]}: {d}")
                        if d.get("status") == "1" and d.get("result","0") != "0":
                            bal = round(int(d["result"]) / 1_000_000, 2)
                            if bal > 0:
                                log.info(f"  ✅ PolygonScan: ${bal:.2f}")
                                return bal
        except Exception as e:
            log.warning(f"  PolygonScan error: {e}")

        # Method 5: MANUAL_BALANCE env var — emergency fallback
        manual = os.getenv("MANUAL_BALANCE", "")
        if manual:
            try:
                bal = float(manual)
                log.info(f"  ✅ Using MANUAL_BALANCE env: ${bal:.2f}")
                return bal
            except: pass

        log.warning(f"⚠️ All balance methods failed for {addr[:10]}")
        return 0.0
    ...
    async def place_order(self, token_id, side, size_usdc, price) -> dict:
        if not self._acct:
            log.info("📄 Paper trade (no wallet)")
            return {"status": "paper", "id": f"paper_{int(time.time())}"}

        # Use maker address if set (Polymarket deposit address), otherwise wallet address
        maker = self.cfg.MAKER_ADDRESS or self._acct.address
...
class TGBot:
    ...
    def _wallet(self):
        ...
        if a:
            return f"👛 <code>{a.address[:6]}...{a.address[-4:]}</code>"
        return "📄 <b>PAPER TRADING</b> (no wallet connected)"

    def _paper_tag(self):
        """Returns paper trading warning if no real wallet."""
        if self.trading_bot.is_paper_trading():
            return "\n⚠️ <i>Paper trading — no real money involved</i>"
        return ""
    ...
    async def alert_daily(self, port: Portfolio, risk: Risk):
        ...
        wr_ok= "✅" if wr >= 0.70 else "⚠️"
        pos  = list(port.positions.values())
        pos_val  = round(sum(x.current_price * x.shares for x in pos), 2)
        pos_pnl  = round(sum(x.pnl for x in pos), 2)
        daily_pct= round(port.daily_pnl / max(1, port.total_value) * 100, 2)
        sign     = "+" if port.daily_pnl >= 0 else ""

        await self.send(
            f"📊 <b>Daily Summary — {utcnow().strftime('%Y-%m-%d')}</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"{self._wallet()}\n\n"
            f"💼 Total Balance: <b>${port.total_value:,.2f}</b>\n"
            f"💵 Available: <b>${port.cash:,.2f}</b>\n"
            f"📂 Positions: <b>{len(pos)}</b> (Value: ${pos_val:.2f})\n"
            f"{'📈' if pos_pnl>=0 else '📉'} Positions PNL: <b>{'+' if pos_pnl>=0 else ''}${pos_pnl:.2f}</b>\n\n"
            f"📈 Today P&L: <b>{sign}${port.daily_pnl:.2f} ({sign}{daily_pct:.2f}%)</b>\n"
            f"🏆 All-time P&L: <b>${port.all_time_pnl:+.2f}</b>\n"
            f"🔍 Signals today: <b>{port.signals_today}</b>\n\n"
            f"📐 <b>Performance</b>\n"
            f"{wr_ok} Win Rate: <code>{wr*100:.1f}%</code> ({port.trades_total} trades) (target 70%)\n"
    ...
    async def _status(self, u, c):
        ...
        pos = list(p.positions.values())
        deployed   = sum(x.size_usd for x in pos)
        pos_val    = round(sum(x.current_price * x.shares for x in pos), 2)
        pos_pnl    = round(sum(x.pnl for x in pos), 2)
        pos_pnl_pct= round(pos_pnl / max(1, deployed) * 100, 2) if pos else 0
        sign       = "+" if pos_pnl >= 0 else ""
        status_icon= "🟢" if b.running else "🔴"

        await u.message.reply_html(
            f"{status_icon} <b>{'RUNNING' if b.running else 'STOPPED'}</b> — Scan #{b._scan_count}\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"{self._wallet()}\n\n"
            f"💼 Total Balance: <b>${p.total_value:,.2f}</b>\n"
            f"💵 Available: <b>${p.cash:,.2f}</b>\n"
            f"📂 Positions: <b>{len(pos)}</b> (Value: ${pos_val:.2f})\n"
            f"{'📈' if pos_pnl>=0 else '📉'} Positions PNL: <b>{sign}${pos_pnl:.2f} ({sign}{pos_pnl_pct:.2f}%)</b>\n\n"
            f"📊 Daily P&L: <b>${p.daily_pnl:+.2f}</b>\n"
            f"🏆 All-time P&L: <b>${p.all_time_pnl:+.2f}</b>\n"
            f"📉 Max Drawdown: <code>{r.max_drawdown()*100:.1f}%</code>"
        )
    ...
    async def _positions(self, u, c):
        ...
        avail_bal    = p.cash
        pos_value    = round(sum(x.current_price * x.shares for x in pos), 2)
        pos_pnl_usd  = round(sum(x.pnl for x in pos), 2)
        pos_pnl_pct  = round(pos_pnl_usd / max(1, sum(x.size_usd for x in pos)) * 100, 2) if pos else 0
        pnl_sign     = "+" if pos_pnl_usd >= 0 else ""

        header = (
            f"🏦 <b>Manage your Positions({len(pos)})</b>\n\n"
            f"Total Balance: <b>${total_val:,.2f}</b>\n"
            f"Available Balance: <b>${avail_bal:,.2f}</b>\n"
            f"Positions Value: <b>${pos_value:,.2f}</b>\n"
            f"Positions PNL: <b>{pnl_sign}${pos_pnl_usd:.2f} ({pnl_sign}{pos_pnl_pct:.2f}%)</b>\n"
        )
        ...
        for i, x in enumerate(pos, 1):
            entry   = x.entry_price
            curr    = x.current_price
            val     = round(curr * x.shares, 2)
            pnl_u   = round(x.pnl, 2)
            pnl_p   = round(x.pnl / max(0.01, x.size_usd) * 100, 1)
            sign    = "+" if pnl_u >= 0 else ""
            emoji   = "📈" if pnl_u >= 0 else "📉"

            lines.append(
                f"\n#{i}. <b>{x.market.question[:60]}</b>\n"
                f"• Side: <b>{x.side}</b> | Exp: {x.opened_at[:10]}\n"
                f"• Invested: <b>${x.size_usd:.2f}</b> • Shares: {x.shares:.2f}\n"
                f"• Entry: <code>${entry:.4f}</code> • Value: <code>${val:.2f}</code>\n"
                f"• SL: <code>${x.stop_loss:.4f}</code>"
                + (f" • TP: <code>${x.take_profit_price:.4f}</code>" if x.take_profit_price else "") + "\n"
                f"• {emoji} PNL: <b>{sign}${pnl_u:.2f} ({sign}{pnl_p:.1f}%)</b>"
            )

        await u.message.reply_html(header + "".join(lines))
...
class TradingBot:
    ...
    def is_paper_trading(self) -> bool:
        """True if no wallet connected = paper trading mode."""
        return self.client._acct is None

    async def sync_balance(self) -> float:
        """Fetch real USDC balance. Real $0 is valid — don't fake it."""
        if self.is_paper_trading():
            log.info("📄 Paper trading mode — no real wallet")
            return self.portfolio.total_value  # keep whatever sim balance

        balance = await self.client.get_usdc_balance()

        # balance=0 is VALID if wallet really has $0
        # Only skip update if fetch completely failed (returned -1 would mean error,
        # but we use 0 as both "real zero" and "failed" — so always trust it)
        deployed = sum(p.size_usd for p in self.portfolio.positions.values())
        self.portfolio.cash        = round(max(0, balance - deployed), 2)
        self.portfolio.total_value = round(balance, 2)
        if balance > self.portfolio.peak_value:
            self.portfolio.peak_value = balance
        log.info(f"✅ Real balance: ${balance:.2f} USDC (cash: ${self.portfolio.cash:.2f} | deployed: ${deployed:.2f})")
        return balance
    ...
    async def start(self):
        ...
        mode_line = "📄 <b>PAPER TRADING MODE</b> — no real money" if is_paper else f"👛 Wallet: <code>{wallet_short}</code>"

        log.info(f"PolyBot v5.2 started — {'PAPER' if is_paper else wallet_short} — STOPPED")
        await self.tg.send(
            f"🤖 <b>PolyBot v5.2 Started</b>\n\n"
            f"{mode_line}\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
