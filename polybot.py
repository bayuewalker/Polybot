...
class PolyClient:
    ...
    async def get_usdc_balance(self) -> float:
        """Fetch real USDC balance - tries multiple sources."""
        if not self._acct:
            return 0.0
        ...
        # Try all USDC contracts on Polygon
        USDC_CONTRACTS = [
        ...
        for rpc_url in RPC_URLS:
            try:
                ...
                for usdc_addr, decimals, label in USDC_CONTRACTS:
                    try:
                        contract = w3.eth.contract(
                            address=Web3.to_checksum_address(usdc_addr), abi=ERC20_ABI
                        )
                        raw = contract.functions.balanceOf(self._acct.address).call()
                        bal = round(raw / (10 ** decimals), 2)
                        if bal > 0:
                            log.info(f"USDC balance ({label}): ${bal} via {rpc_url}")
                            total += bal
                    except Exception as e:
                        log.debug(f"  {label} error: {e}")

                if total >= 0:
                    log.info(f"Total USDC balance: ${total}")
                    return total
        ...
        # Fallback: try Polymarket CLOB API for available collateral
        try:
            ...
            async with self.session.get(url, params=params, headers=headers, timeout=8) as r:
                if r.status == 200:
                    data = await r.json()
            ...
            log.debug(f"Polymarket balance fallback error: {e}")

        log.warning("Could not fetch USDC balance from any source")
        return 0.0
...
class WebAPI:
    ...
    async def handle_balance(self, req):
        """Force sync real balance and return it."""
        balance = await self.bot.sync_balance()
        return self.cors(aioWeb.Response(
            text=json.dumps({"balance": balance, "portfolio": self.bot.portfolio.total_value,
