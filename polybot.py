...
class TGBot:
    ...
    async def alert_trade(self, pos: Position, action: str, reason: str=""):
        w = self._wallet()
        if action == "open":
            tp = f"<code>{pos.take_profit_price:.4f}</code>" if pos.take_profit_price else "<code>hold to resolve</code>"
            await self.send(
                f"✅ <b>Trade Opened</b>\n"
                f"━━━━━━━━━━━━━━━━━━━━\n"
                ...
                f"SL: <code>{pos.stop_loss:.4f}</code> | TP: {tp}\n"
                f"━━━━━━━━━━━━━━━━━━━━\n"
                f"{w} | 🕐 {utcnow().strftime('%Y-%m-%d %H:%M UTC')}"
            )
        else:
            ...
            await self.send(
                f"{emoji} <b>Trade Closed{reason_str}</b>\n"
                f"━━━━━━━━━━━━━━━━━━━━\n"
                f"📊 <b>{pos.market.question[:60]}…</b>\n\n"
