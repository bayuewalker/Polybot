...
LATEST UPDATE — v4.2 (today):
  ✅ FIXED: Bot crash — handle_start missing from WebAPI
  ✅ FIXED: Telegram Conflict — delete_webhook before polling
  ✅ FIXED: Settings not saving — BOT_SETTINGS env var fallback
  ✅ FIXED: SL/TP wrong direction for NO positions
  ✅ FIXED: Balance fetch — pure async eth_call (no blocking Web3)
  ✅ FIXED: Balance tries 4 RPC + PolygonScan fallback
  ✅ NEW: Bot starts STOPPED by default — user must press ▶
  ✅ NEW: Wallet address shown in all Telegram notifications
  ✅ NEW: TP/SL configurable from Settings page
  ✅ NEW: /balance endpoint for manual balance refresh
  ✅ NEW: ⟳ refresh button on portfolio strip
...
v4.0:
  ...
  ✅ 5 strategies: Arbitrage, News, MeanRev, VolumeSpike, ClosingSoon
"""
...
def load_file() -> dict:
    """Load settings — file first, then BOT_SETTINGS env var fallback."""
    try:
    ...
    # Fallback: BOT_SETTINGS env var (JSON string) for Railway persistence
    env_s = os.getenv("BOT_SETTINGS", "")
    if env_s:
        try: return json.loads(env_s)
        except Exception: pass
    return {}
...
def save_file(data: dict):
    try:
        ...
        log.info(f"✅ Settings saved: {[k for k in data if k != 'privateKey']}")
    except Exception as e:
...
class Config:
    ...
    def reload(self):
        s = load_file()
        log.info(f"Config reload — settings.json: {json.dumps({k:v for k,v in s.items() if k != 'privateKey'}, indent=None)}")
        ...
        def gf(key, env, default):
            ...
            if v is not None and str(v).strip():
                try:
                    result = float(v)
                    log.info(f"  {key} = {result} (from settings.json)")
                    return result
                except:
                    pass
            env_val = os.getenv(env)
            if env_val:
                log.info(f"  {key} = {float(env_val)} (from env {env})")
                return float(env_val)
            log.info(f"  {key} = {default} (default)")
        ...
        def gi(key, env, default):
            ...
            return int(float(os.getenv(env, str(default))))
        ...
        log.info(f"TP: {f'{self.TP_PCT*100:.0f}%' if self.TP_PCT else 'hold to resolve'} | "
                 f"SL: {f'{self.SL_PCT*100:.0f}%' if self.SL_PCT else f'{self.STOP_LOSS*100:.0f}% (default)'}")
        log.info(f"MAX_POSITION=${self.MAX_POSITION} | DAILY_LIMIT=${self.DAILY_LIMIT} | MAX_POSITIONS={self.MAX_POSITIONS}")
...
class PolyClient:
    ...
    async def start(self):
        self.session = aiohttp.ClientSession(headers={"User-Agent": "PolyBot/4.2"})

    async def get_usdc_balance(self) -> float:
        """Fetch USDC balance from Polymarket API — most reliable from Railway."""
        if not self._acct:
            return 0.0
        ...
        # Method 1: Polymarket CLOB balance-allowance (works from Railway)
        try:
            url = f"{self.cfg.CLOB}/balance-allowance"
            params = {"asset_type": "COLLATERAL", "account": addr}
            async with self.session.get(url, params=params, timeout=10) as r:
                log.info(f"  CLOB balance status: {r.status}")
                if r.status == 200:
                    data = await r.json()
                    log.info(f"  CLOB response: {data}")
                    raw = float(data.get("balance", 0) or data.get("allowance", 0) or 0)
                    # CLOB returns raw units (6 decimals) or actual float
                    bal = raw / 1_000_000 if raw > 1000 else raw
                    if bal > 0:
                        log.info(f"  ✅ CLOB balance: ${bal:.2f}")
                        return round(bal, 2)
        except Exception as e:
            log.warning(f"  CLOB balance error: {e}")

        # Method 2: Polymarket Gamma API profile
        try:
            url = f"{self.cfg.GAMMA}/markets"
            params = {"maker": addr, "limit": 1}
            async with self.session.get(url, params=params, timeout=8) as r:
                log.info(f"  Gamma profile status: {r.status}")
        except Exception as e:
            log.warning(f"  Gamma error: {e}")

        # Method 3: Direct JSON-RPC eth_call (try only public RPCs)
        ...
        for rpc in RPC_URLS:
            try:
                for contract, label in USDC_CONTRACTS:
                    ...
                    async with self.session.post(rpc, json=payload, timeout=6) as r:
                        if r.status == 200:
                            resp = await r.json()
                            ...
                            if result and result != "0x" and len(result) > 2:
                                ...
                                if bal > 0:
                                    log.info(f"  ✅ {label} via {rpc}: ${bal}")
                                    return bal
            except Exception as e:
                log.debug(f"  RPC {rpc}: {e}")

        log.warning(f"All balance methods failed — check Railway network access")
        return 0.0
...
class Risk:
    ...
    def can_trade(self):
        # ── Safety: refuse to trade if settings not saved by user ────────────
        s = load_file()
        if not s.get("_confirmed"):
            return False, "Settings not confirmed — user must Save settings from dashboard first"
        if not s.get("privateKey"):
            return False, "No private key — configure in dashboard Settings"
        # ── Risk checks ───────────────────────────────────────────────────────
    ...
    def stop(self, entry, side):
        # For BOTH YES and NO tokens: loss = price drops below entry
        # YES token: buy at 0.5, SL at 0.40 (price drops = bad)
        # NO token:  buy at 0.5, SL at 0.40 (price drops = bad)
        sl = self.cfg.SL_PCT if self.cfg.SL_PCT else self.cfg.STOP_LOSS
        return round(entry * (1 - sl), 4)

    def take_profit(self, entry, side):
        # TP = price rises above entry by TP%
        # NO TP triggered when NO token price rises (toward 1.0)
        if not self.cfg.TP_PCT:
            return None
        return round(entry * (1 + self.cfg.TP_PCT), 4)

    def check_stops(self):
        """Returns list of (pid, reason) that hit SL or TP."""
        to_close = []
        for pid, p in self.port.positions.items():
            # Stop loss: price dropped below SL level (same for YES and NO)
            if p.current_price <= p.stop_loss:
                to_close.append((pid, "stop_loss"))
            # Take profit: price rose above TP level
...
class TGBot:
    def __init__(self, cfg, bot):
        self.cfg=cfg; self.bot=bot; self.trading_bot=bot; self.app=None
    ...
    def _wallet_tag(self):
        """Returns short wallet string for TG messages."""
        acct = self.trading_bot.client._acct
        if acct:
            addr = acct.address
            return f"👛 <code>{addr[:6]}...{addr[-4:]}</code>"
        return "👛 <code>No wallet</code>"
    ...
    async def alert_signal(self, sig):
        ...
        await self.send(
            ...
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"{self._wallet_tag()} | 🕐 {utcnow().strftime('%Y-%m-%d %H:%M UTC')}"
        )

    async def alert_trade(self, pos, action):
        wallet = self._wallet_tag()
        if action=="open":
            tp_line = f"TP: <code>{pos.take_profit_price:.4f}</code>" if pos.take_profit_price else "TP: <code>Hold to resolve</code>"
            await self.send(
                f"✅ <b>Trade Opened</b>\n"
                f"━━━━━━━━━━━━━━━━━━━━\n"
                ...
                f"SL: <code>{pos.stop_loss:.4f}</code> | {tp_line}\n"
                f"━━━━━━━━━━━━━━━━━━━━\n"
                f"{wallet} | 🕐 {utcnow().strftime('%Y-%m-%d %H:%M UTC')}"
            )
        else:
            ...
            await self.send(
                ...
                f"━━━━━━━━━━━━━━━━━━━━\n"
                f"📊 <b>{pos.market.question[:60]}…</b>\n\n"
                f"P&L: <b>{pnl_str}</b> | Entry: <code>{pos.entry_price:.4f}</code>\n"
...
class WebAPI:
    ...
    async def handle_post_settings(self, req):
        try:
            ...
            return self.cors(aioWeb.Response(status=400, text=f'{{"error":"Invalid JSON: {e}"}}'))

        log.info(f"POST /settings received keys: {[k for k in data.keys() if k != 'privateKey']}")
        ...
        verify = load_file()
        log.info(f"Settings verify after save: maxPosition={verify.get('maxPosition')} slPct={verify.get('slPct')}")
    ...
    async def handle_balance(self, req):
        """Force sync real balance and return debug info."""
...
class TradingBot:
    ...
    async def sync_balance(self):
        """Fetch real USDC balance and update portfolio."""
        ...
        if balance > 0:
            ...
            log.info(f"Portfolio synced: ${balance:.2f} USDC (cash: ${self.portfolio.cash:.2f})")
        return balance

    async def start(self):
        await self.client.start()

        # Sync real USDC balance from wallet
        await self.sync_balance()

        # Start web API server
        await self.api.start(self.cfg.API_PORT)

        wallet_short = f"{self.client._acct.address[:6]}...{self.client._acct.address[-4:]}" if self.client._acct else "⚠️ Not connected"
        log.info(f"PolyBot v4.2 started — wallet: {wallet_short} — STOPPED")
        await self.tg.send(
            f"🤖 <b>PolyBot v4.2 Started</b>\n\n"
            f"👛 Wallet: <code>{wallet_short}</code>\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"⚡ Arbitrage | 📰 News | 📊 MeanRev | 🔊 Volume | ⏰ Closing\n"
            f"🌐 Web API: active\n"
        ...
        while True:
            try:
                if self.running:
                    await self.scan()
                # Sync real balance every 2 scans (~2 minutes)
    ...
    async def scan(self):
        ...
        log.info(f"Scan #{self._scan_count} | MaxPos=${self.cfg.MAX_POSITION} | "
                 f"SL={self.cfg.SL_PCT*100 if self.cfg.SL_PCT else self.cfg.STOP_LOSS*100:.0f}% | "
                 f"TP={f'{self.cfg.TP_PCT*100:.0f}%' if self.cfg.TP_PCT else 'none'} | "
